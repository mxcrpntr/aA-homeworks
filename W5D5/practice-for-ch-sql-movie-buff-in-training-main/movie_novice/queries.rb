...
def top_titles
  ...
  Movie.select(:title).where('score >= 9', true)
end
...
def below_average_years
  ...
  Movie.select('yr,COUNT(*) AS bad_movies').where('score < 5', true).group(:yr).order('bad_movies DESC')
end
...
def pulp_fiction_actors
  ...
  Actor.select('actors.id', :name).joins(:movies).where('movies.title = ?', 'Pulp Fiction')
end
